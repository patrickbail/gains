...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    if args.resolution in [1, 2, 4, 8]:
        resolution = math.floor(orig_w/(resolution_scale * args.resolution) + 0.5), math.floor(orig_h/(resolution_scale * args.resolution) + 0.5)
    ...
    if len(cam_info.image.split()) > 3:
        ...
        if cam_info.mask is not None:
            loaded_mask = torch.tensor(cam_info.mask)
        else:
            loaded_mask = None
        gt_image = resized_image_rgb
    ...
    if cam_info.mono_depth is not None:
        mono_depth = torch.tensor(cam_info.mono_depth).detach()

    mono_normal = None
    if cam_info.mono_normal is not None:
        mono_normal = torch.tensor(cam_info.mono_normal).detach()
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  albedo=cam_info.albedo, roughness=cam_info.roughness, metallic=cam_info.metallic, 
                  image_path=cam_info.image_path, mono_depth=mono_depth, mono_normal=mono_normal)
