...
def generate_path(viewpoint_cameras, n_frames=480):
  ...
  for c2w in new_poses:
      ...
      R_new = w2c[:3, :3].T
      #R_new = w2c[:3, :3] @ np.diag([-1, 1, 1]) # wrong one
      C_new = c2w[:3, 3]
      T_new = -R_new @ C_new
      cam = Camera(viewpoint_cameras[0].colmap_id, R_new, T_new, viewpoint_cameras[0].FoVx,
                   viewpoint_cameras[0].FoVy, None, None, viewpoint_cameras[0].image_name, viewpoint_cameras[0].uid,
                   None, None, HWK=viewpoint_cameras[0].HWK, image_path=viewpoint_cameras[0].image_path)
      ...
      cam.calc_rays()
      #cam.R = torch.from_numpy(R_new).float().cuda()
      #cam.T = torch.from_numpy(T_new).float().cuda()
      traj.append(cam)
...
def create_videos(base_dir, input_dir, out_name, num_frames=480, render_seg=False, indirect_color=False):
  ...
  rgb_file = os.path.join(input_dir, 'renders', f'render_{idx_to_str(0)}.png')
  rgb_frame = load_img(rgb_file)
  shape = rgb_frame.shape
  p = 3
  h, w = rgb_frame.shape[:2]

  # --- Fix shape BEFORE creating writer ---
  new_h = h + (h % 2)
  new_w = w + (w % 2)
  if (new_h != h) or (new_w != w):
      print(f"Padding video shape from {(h, w)} to even dimensions {(new_h, new_w)} for H.264 compatibility.")
  shape = (new_h, new_w, rgb_frame.shape[2])
  # ---------------------------------------
  ...
  render_options = ['render', 'depth', 'base', 'diff', 'spec', 'refl', 'roughness', 'alpha', 'normal', 'surfnormal', 'render_env']
  if render_seg:
    render_options.append('seg')
  if indirect_color:
    render_options.append('indirect')
  
  for k in render_options:
    video_file = os.path.join(base_dir, f'{video_prefix}_{k}.mp4')
    input_format = 'rgb' # gray' if k == 'alpha' else 'rgb'
    

    file_ext = 'png' # 'png' if k in ['color', 'normal'] else 'tiff'
    idx = 0
    ...
    file0 = os.path.join(input_dir, 'renders', f'render_{idx_to_str(0)}.{file_ext}')

    if not os.path.exists(file0):
      print(f'Images missing for tag {k}')
      continue

    file0k = os.path.join(input_dir, 'renders', f'{k}_{idx_to_str(0)}.{file_ext}')
    if not os.path.exists(file0k):
      print(f'Tag {k} does not exist')
      continue
    ...
    with media.VideoWriter(
      ...
      for idx in tqdm(range(num_frames)):
        if k == "gt":
          img_file = os.path.join(input_dir, 'gt', f'{idx_to_str(idx)}.{file_ext}')
        else:
          img_file = os.path.join(input_dir, 'renders', f'{k}_{idx_to_str(idx)}.{file_ext}')
        #if k == 'color':
        ...
        # ---- NEW SECTION: enforce even width & height ----
        h, w = img.shape[:2]
    ...
    # Create GIFs
    """
    os.makedirs(os.path.join(base_dir, "gif"), exist_ok=True)
    gif_path = os.path.join(base_dir, "gif", f"{video_prefix}_{k}.gif")

    print(f"Converting {video_file} → {gif_path} ...")
    palette = os.path.join(base_dir, "gif", "palette.png")
