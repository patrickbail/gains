...
renderCUDA(
	...
	const int S, int W, int H, const int K, const int KP, const int KS,
	float focal_x, float focal_y,
	const float2* __restrict__ points_xy_image,
	const float* __restrict__ colors,
	const float* __restrict__ features,
	const float* __restrict__ seg_ids,
	const float* __restrict__ seg_ids_part,
	const float* __restrict__ seg_ids_subpart,
	...
	bool render_seg)
{
	...
	//float* SG = nullptr;
    //if (render_seg) {
    //    SG = shared_mem + threadIdx.x * K;
    //    // zero init
    //    for (int ch = 0; ch < K; ++ch)
    //        SG[ch] = 0.0f;
    //}
	float SG[SEG_MAX_CHANNELS] = { 0 };
	float SGP[SEG_MAX_CHANNELS] = { 0 };
	float SGS[SEG_MAX_CHANNELS] = { 0 };
	float weight_max=0;
	int idx_max=0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (render_seg)
			{
				float w = alpha * T;
				for (int ch = 0; ch < K; ch++)
					SG[ch] += seg_ids[collected_id[j] * K + ch] * w;
				for (int ch = 0; ch < KP; ch++)
					SGP[ch] += seg_ids_part[collected_id[j] * KP + ch] * w;
				for (int ch = 0; ch < KS; ch++)
					SGS[ch] += seg_ids_subpart[collected_id[j] * KS + ch] * w;
			}
	...
	if (inside)
	{
		...
		{
			for (int ch = 0; ch < K; ch++)
				seg[ch * H * W + pix_id] = SG[ch];
			for (int ch = 0; ch < KP; ch++)
				seg_p[ch * H * W + pix_id] = SGP[ch];
			for (int ch = 0; ch < KS; ch++)
				seg_s[ch * H * W + pix_id] = SGS[ch];
		}
		accum_max_count[pix_id] = idx_max;
...
void FORWARD::render(
	...
	const float* seg_ids,
	const float* seg_ids_part,
	const float* seg_ids_subpart,
	...
	bool render_seg)
{
	// Probably causing issues
	//size_t shmem_size = 0;
    //if (render_seg)
    //    shmem_size = block.x * block.y * K * sizeof(float);
	//renderCUDA<NUM_CHANNELS> << <grid, block, shmem_size >> > (
