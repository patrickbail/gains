...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& seg_ids,	//
	const torch::Tensor& seg_ids_part,	//
	const torch::Tensor& seg_ids_subpart,	//
	...
	const bool render_seg)
{
  ...
  const int K = seg_ids.size(1);	// //
  const int KP = seg_ids_part.size(1);	// //
  const int KS = seg_ids_subpart.size(1);	// //
  ...
  torch::Tensor accum_max_count = torch::full({1, H, W}, 0, int_opts); //
  torch::Tensor seg = torch::full({K, H, W}, 0.0, float_opts); //
  torch::Tensor seg_p = torch::full({KP, H, W}, 0.0, float_opts); //
  torch::Tensor seg_s = torch::full({KS, H, W}, 0.0, float_opts); //
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		seg_ids.contiguous().data_ptr<float>(), //
		seg_ids_part.contiguous().data_ptr<float>(), //
		seg_ids_subpart.contiguous().data_ptr<float>(), //
		...
		seg.contiguous().data_ptr<float>(), //
		seg_p.contiguous().data_ptr<float>(), //
		seg_s.contiguous().data_ptr<float>(), //
		radii.contiguous().data_ptr<int>(),
		debug,
		render_seg);
  }
  return std::make_tuple(rendered, out_contrib, out_color, out_feature, out_others, accum_max_count, seg, seg_p, seg_s, radii, geomBuffer, binningBuffer, imgBuffer);
}
