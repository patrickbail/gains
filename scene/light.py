...
class EnvLight(torch.nn.Module):
    ...
    def load(self, path):
        ...
        if path.endswith(".exr"):
            hdr_image = pyexr.open(path).get()[:, :, :3]
        else:
    ...
    def __call__(self, l, mode=None, roughness=None, nv_cam=False):
        """
        ...
        if mode == "diffuse":
            # Diffuse lighting
            light = dr.texture(self.diffuse[None, ...] if not nv_cam else self.diffuse[None, ...].detach(),
                                l, filter_mode='linear', boundary_mode='cube')
        elif mode == "pure_env":
            # Pure environment light (no mip-map)
            light = dr.texture(self.base[None, ...] if not nv_cam else self.base[None, ...].detach(),
                                l, filter_mode='linear', boundary_mode='cube')
        else:
            # Specular lighting with mip-mapping
            miplevel = self.get_mip(roughness)
            light = dr.texture(
                self.specular[0][None, ...] if not nv_cam else self.specular[0][None, ...].detach(), 
                l,
